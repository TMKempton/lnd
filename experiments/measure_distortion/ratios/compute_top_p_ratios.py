...
from src.distributions import compute_p_distribution, compute_top_p_q_distribution
...
if __name__ == "__main__":
    ...
    for initial_context in initial_contexts:
        ...
        for _ in range(n_repetitions):
            ...
            p_dist_1 = compute_p_distribution(completion=completions[0], llm=llm)
            q_dist_1 = compute_top_p_q_distribution(
                completion=completions[0],
                top_p=top_p,
                llm=llm,
            )
            p_dist_2 = compute_p_distribution(completion=completions[1], llm=llm)
            q_dist_2 = compute_top_p_q_distribution(
            ...
            logger.info(f"q(T_1) (top-k local-norm): {q_dist_1}")
            logger.info(f"q(T_2) (top-k local-norm): {q_dist_2}")

            p_ratio = np.prod(
                [
                    p_dist_1[idx] / p_dist_2[idx]
                    for idx in range(len(p_dist_1))
                    if p_dist_1[idx] and p_dist_2[idx]
                ]
            )
            q_ratio = np.prod(
                [
                    q_dist_1[idx] / q_dist_2[idx]
                    for idx in range(len(q_dist_1))
                    if q_dist_1[idx] and q_dist_2[idx]
                ]
            )
    ...
    with open(result_dir / f"top_p={top_p}_ratios_long.json", "w") as f:
