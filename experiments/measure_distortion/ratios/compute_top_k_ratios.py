...
if __name__ == "__main__":
    ...
    for initial_context in initial_contexts:
        ...
        for idx in range(n_repetitions):
            logger.info(f"Experiment {idx} of {n_repetitions}")
            ...
            p_dist_1 = compute_p_distribution(completions[0], llm=llm)
            q_dist_1 = compute_top_k_q_distribution(
                completion=completions[0],
                top_k=top_k,
                llm=llm,
            )
            p_dist_2 = compute_p_distribution(completions[1], llm=llm)
            q_dist_2 = compute_top_k_q_distribution(
    ...
    with open(result_dir / f"top_k={top_k}_ratios_long.json", "w") as f:
