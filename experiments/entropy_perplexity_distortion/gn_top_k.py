...
from src.distributions import compute_p_distribution, compute_top_k_next_token_probs
from src.llm import LLM
from src.hardware import DEVICE

def is_valid_top_k_completion(
...
def filter_completions(
    completions: List[str], llm: LLM, top_k: int
) -> List[str]:
    return [c for c in completions if is_valid_top_k_completion(c, llm, top_k)]
...
if __name__ == "__main__":
    ...
    for initial_context in initial_contexts:
        ...
        with open(artefact_dir / f"gn_top_k={top_k}_completion.json", "w") as f:
            f.write(
                json.dumps(
    ...
    with open(result_dir / f"gn_top_k={top_k}_metrics.json", "w") as f:
